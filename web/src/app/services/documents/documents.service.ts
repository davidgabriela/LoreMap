...
export class DocumentsService {
  ...
  getDocumentsFromLore(loreId: string): Observable<Document[]> {
    const documentsUrl = `https://lore-map.herokuapp.com/api/v1/lore-collection/${loreId}/documents/`;
  ...
  getDocumentById(loreId: string, documentId: string): Observable<Document> {
    const documentUrl = `https://lore-map.herokuapp.com/api/v1/lore-collection/${loreId}/documents/${documentId}`;
    return this.http.get(documentUrl).pipe(map((data: any) => data.data));
  }
  createDocument(document: Document) {
    const createUrl = 'https://lore-map.herokuapp.com/api/v1/documents';
    return this.http.post(createUrl, document);
  }
  updateDocument(id: string, htmlContent: string) {
    const updateUrl = `https://lore-map.herokuapp.com/api/v1/documents/${id}`;
    const body = {
      content: htmlContent,
    };
